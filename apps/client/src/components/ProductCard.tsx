...
const ProductCard = ({ product, page = "home" }: ProductCardProps) => {
    return (
        <Link to={`/product/${product.id}`}>
            <div className={`bg-white ${page === "home" ? "min-w-32" : "min-w-52"} max-w-72 border h-96 cursor-pointer rounded-sm overflow-hidden flex flex-col`}>
                <div className="w-full h-[44rem] bg-neutral-200">
                    ...
                    <div className="flex gap-2 items-center">
                        {product.promotionPrice ? (
                            <div className="flex flex-col md:flex-row-reverse md:items-center md:gap-2">
                                ...
                                <p className="font-semibold text-xl">{getFormattedPrice(product.promotionPrice)}</p>
                            </div>
                        ) : (
                            <p className="font-semibold text-xl">{getFormattedPrice(product.price)}</p>
                        )}
                    </div>
                </div>
            </div>
